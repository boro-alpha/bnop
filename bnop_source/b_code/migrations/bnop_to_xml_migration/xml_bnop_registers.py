...
class XmlBnopRegisters:
    ...
    def update_migration_uow(
            ...
            bnop_objects: list):
        XmlBnopRegisters.__migrated_bnop_objects_keyed_by_ck_id[boro_ckid_type] = \
            bnop_objects
    ...
    def try_to_add_to_branches(
        ...
        if bnop_parent in XmlBnopRegisters.__xml_model_branches:
            if bnop_object in XmlBnopRegisters.__xml_model_branches[bnop_parent]:
                return \
                    False

            else:
                XmlBnopRegisters.__xml_model_branches[bnop_parent].append(
                    bnop_object)

        else:
            success = \
                XmlBnopRegisters.__try_to_add_to_trunks(
    ...
    def __try_to_add_to_trunks(
        ...
        for branch_root, branch_trunk in XmlBnopRegisters.__xml_model_branches.items():
            added_to_trunk = \
                XmlBnopRegisters.__try_to_add_to_trunk(
                    bnop_object=bnop_object,
                    bnop_parent=bnop_parent,
        ...
        if not added_to_trunk:
            XmlBnopRegisters.__xml_model_branches[bnop_parent] = \
                [bnop_object]
    ...
    def __try_to_add_to_trunk(
        ...
        if bnop_parent in branch_trunk:
            if bnop_object in XmlBnopRegisters.__xml_model_branches[branch_root]:
                return \
                    False

            else:
                XmlBnopRegisters.__xml_model_branches[branch_root].append(
                    bnop_object)
