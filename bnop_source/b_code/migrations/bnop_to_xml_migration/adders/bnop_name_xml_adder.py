...
from bnop_source.b_code.migrations.bnop_to_xml_migration.adders.bnop_object_xml_adder import add_bnop_object_types_to_xml_tree
...
def add_bnop_name_to_xml_tree(
    ...
    if configuration.include_presentation_names:
        name_tree_element = \
            etree.SubElement(
                parent_element,
...
def __add_bnop_named_by_objects_to_xml_tree(
    ...
    for bnop_named_object in bnop_name.names:
        if configuration.include_presentation_names:
            etree.SubElement(
                named_by_tree_element,
                BoroObjectCkIds.Objects.name,
                uuid=str(bnop_named_object.uuid),
